<template>
    ...
    <div class="form-check form-check-inline" v-for="opt in ['subjects', 'quizzes']" :key="opt">
    ...
    <ul class="list-group mt-3">
      <li v-for="(result, index) in results" :key="index" class="list-group-item">
        <span v-if="result.type === 'subject'"> Subject: {{ result.name }}</span>
        <span v-else-if="result.type === 'quiz'"> Quiz: {{ result.title }}</span>
      </li>
    </ul>
  </div>
</template>
